#include "Matrix_inc.h"

#include <iostream>
#include <limits>
#include <algorithm>

using std::cout;
using std::vector;
using std::fstream;

Matrix_inc::Matrix_inc()
{}

Matrix_inc::Matrix_inc(Adjency_list &list)
{
    vector < vector < int > > temp_list = list.get_list();

    int number_of_edges = 0;

    for(int row = 0; row < (int)temp_list.size(); row++)
        number_of_edges += temp_list[row].size();

    number_of_edges /= 2;

    for(int row = 0; row < (int)temp_list.size(); row++)
    {
        matrix.push_back(vector < int >());

        for(int col = 0; col < number_of_edges; col++)
            matrix[row].push_back(0);
    }

    int edge = 0;

    for(int row = 0; row < (int)temp_list.size(); row++)
    {
        for(int col = 0; col < (int)temp_list[row].size(); col++)
        {
            matrix[row][edge] = 1;
            int second_vertice = temp_list[row][col];
            matrix[second_vertice][edge] = 1;
            temp_list[second_vertice].erase(std::find(temp_list[second_vertice].begin(), temp_list[second_vertice].end(), row));
            edge++;
        }
        if(edge > 7)
            break;
    }
}

void Matrix_inc::read_matrix(fstream &file)
{
    char find_endline;
    int temp, n = 0;

    file.get();
    file.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

    while(!file.eof())
    {
        matrix.push_back(vector < int >());
        file.ignore(2);

        find_endline = file.get();
        while(find_endline == '\t')
        {
            file >> temp;
            matrix[n].push_back(temp);
            find_endline = file.get();
        }

        n++;
    }
}

void Matrix_inc::print_matrix_inc()
{
    cout << "  \t";
    for(int col = 0; col < (int)matrix[0].size(); col++)
        cout << "L" << col << "\t";
    for(int col = 0; col < (int)matrix.size(); col++)
    {
        cout << "\n";
        cout << col << ":\t";
        for(int row = 0; row < (int)matrix[col].size(); row++)
        {
            cout << matrix[col][row] << "\t";
        }
    }
    cout << '\n';
}

vector < vector < int > > Matrix_inc::get_matrix() const
{
    return matrix;
}
